...
public class weaponselector : MonoBehaviour 
{
	...
	void Update()
	{
		...
		if (ControlFreak2.CF2Input.GetButtonDown("Inventory") && canswitch && !playercontrol.climbladder)
		{
		...
		if (inventoryOn)
		{
			...
			if(ControlFreak2.CF2Input.GetAxis("Horizontal")<0 && Time.time > nextselect && canswitch && !hideweapons)
			{
			...
			else if(ControlFreak2.CF2Input.GetAxis("Horizontal")>0 && Time.time > nextselect && canswitch && !hideweapons)
			{
				nextselect = Time.time + selectInterval;
				//go right
				myaudio.clip = uiswitchsound;
			...
			if(ControlFreak2.CF2Input.GetAxis("Vertical")>0 && Time.time > nextselect && canswitch && !hideweapons)
			{
				nextselect = Time.time + selectInterval;
				//go up
			...
			else if(ControlFreak2.CF2Input.GetAxis("Vertical")<0 && Time.time > nextselect && canswitch && !hideweapons)
			{
				nextselect = Time.time + selectInterval;
				//go down
				myaudio.clip = uiswitchsound2;
