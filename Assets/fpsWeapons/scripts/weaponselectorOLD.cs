...
public class weaponselectorOLD : MonoBehaviour {
	...
	void Update () 
	{
		...
		if(ControlFreak2.CF2Input.GetAxis("CycleWeapons")>0 && Time.time > nextselect && canswitch 
		   || 
		   (ControlFreak2.CF2Input.GetButtonDown ("CycleWeapons") && Time.time > nextselect && canswitch && !hideweapons))
		...
		else if(ControlFreak2.CF2Input.GetAxis("CycleWeapons")<0 && Time.time > nextselect && canswitch && !hideweapons)
		{
			nextselect = Time.time + selectInterval;
